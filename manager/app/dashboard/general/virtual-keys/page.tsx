...
export default function ApiKeysPage() {
  ...
  const handleUpdateKey = async () => {
    ...
    setUpdating(true);
    try {
      const body: VirtualKey = {
        ...editingKey,
        name: editForm.name.trim() || "Default",
      ...
      const updated = await adminFetch<VirtualKey>(`/admin/virtual_keys/${encodeURIComponent(editingKey.key)}`, {
        method: "PUT",
        body: JSON.stringify(body),
      });
      setApiKeys((prev) => prev.map((item) => (item.key === editingKey.key ? updated : item)));
  ...
  const handleDelete = async () => {
    ...
    const pendingKey = apiKeys.find((apiKey) => apiKey.key === pendingDeleteKey);
    if (pendingKey?.read_only) {
    ...
    setDeleting(true);
    try {
      await adminFetch(`/admin/virtual_keys/${encodeURIComponent(pendingDeleteKey)}`, {
        method: "DELETE",
      });
      setApiKeys((prev) => prev.filter((k) => k.key !== pendingDeleteKey));
    ...
    <div className="space-y-4">
      ...
      {loading ? (
        ...
        <div className="overflow-x-auto">
          <section className="min-w-[700px] overflow-hidden rounded-lg border border-slate-700/70 bg-slate-900/40">
            ...
            {apiKeys.map((apiKey) => (
              <div key={apiKey.key} className="grid grid-cols-[minmax(0,1fr)_180px_160px_230px] items-center border-b border-slate-700/60 px-3 py-2 last:border-b-0">
                <div className="min-w-0">
                  <p className="truncate text-xs font-medium text-slate-100">{apiKey.name || "(unnamed)"}</p>
                ...
                <div className="flex justify-end gap-1.5">
                  ...
                  <span title={apiKey.read_only ? "Read-only virtual key cannot be deleted" : undefined}>
                    <Button
                      variant="danger"
                      onClick={() => { setPendingDeleteKey(apiKey.key); setShowConfirm(true); }}
                      disabled={!!apiKey.read_only}
                      className="px-2.5 py-1 text-xs"
                    >
                      Delete
      ...
      <Modal isOpen={isCreateModalOpen} onClose={() => setIsCreateModalOpen(false)}>
        ...
        <ModalFooter>
          ...
          <Button onClick={handleCreateKey} disabled={creating || !form.name.trim()}>
      ...
      >
        ...
        <ModalFooter>
          ...
          <Button onClick={handleUpdateKey} disabled={updating || !editForm.name.trim()}>
      ...
      <ConfirmDialog
        isOpen={showConfirm}
        onClose={() => { setShowConfirm(false); setPendingDeleteKey(null); }}
