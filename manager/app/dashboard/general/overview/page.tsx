...
const statusCards = [
  ...
  { label: "Virtual Keys", value: "2 active", tone: "text-slate-100", icon: "♟", iconTone: "text-amber-300" },
    ...
    <div className="space-y-4">
      <section className="rounded-lg border border-slate-700/70 bg-slate-900/40 p-4">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            <Link href="/dashboard/llm/providers" className="rounded-md border border-slate-600/80 bg-slate-800/70 px-3 py-1.5 text-xs font-semibold uppercase tracking-[0.1em] text-slate-200 transition-colors hover:bg-slate-700/80">
              Providers
            </Link>
            <Link href="/dashboard/general/virtual-keys" className="rounded-md border border-slate-600/80 bg-slate-800/70 px-3 py-1.5 text-xs font-semibold uppercase tracking-[0.1em] text-slate-200 transition-colors hover:bg-slate-700/80">
              Virtual Keys
            </Link>
            <Link href="/dashboard/configuration/gateway" className="rounded-md border border-slate-600/80 bg-slate-800/70 px-3 py-1.5 text-xs font-semibold uppercase tracking-[0.1em] text-slate-200 transition-colors hover:bg-slate-700/80">
              Gateway
            </Link>
          </div>
        </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <section className="rounded-lg border border-slate-700/70 bg-slate-900/40 p-4">
          ...
          <div className="space-y-2">
            {[
              { step: "1", text: "Configure a provider (OpenAI, Anthropic, etc.)", href: "/dashboard/llm/providers" },
              { step: "2", text: "Create a virtual key for authentication", href: "/dashboard/general/virtual-keys" },
              { step: "3", text: "Configure the gateway before routing traffic", href: "/dashboard/configuration/gateway" },
            ].map((item) => (
      ...
      <section className="rounded-lg border border-slate-700/70 bg-slate-900/40 p-4">
        ...
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-6">
          {[
            { label: "Providers", href: "/dashboard/llm/providers", desc: "LLM backends" },
            { label: "Models", href: "/dashboard/llm/models", desc: "Model catalog" },
            { label: "Virtual Keys", href: "/dashboard/general/virtual-keys", desc: "Auth tokens" },
          ].map((mod) => (
